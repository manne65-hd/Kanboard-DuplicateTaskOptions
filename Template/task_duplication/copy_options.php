<div class="page-header"><!-- copied from /app/Template/task_duplication/copy.php -->
    <h2><?= t('Duplicate task with options') ?></h2>
</div>
...
<?php else: ?>
    ...
    <form method="post" action="<?= $this->url->href('DuplicateTaskOptionsController', 'copyOptions', array('task_id' => $task['id'], 'project_id' => $task['project_id'])) ?>" autocomplete="off">
        <?= $this->form->csrf() ?>
        ...
        <?= $this->app->component('select-dropdown-autocomplete', array(
            ...
            'replace'      => array(
                'regex' => 'PROJECT_ID',
                'url' => $this->url->href('DuplicateTaskOptionsController', 'copyOptions', array('task_id' => $task['id'], 'project_id' => $task['project_id'], 'dst_project_id' => 'PROJECT_ID')),
            )
        )) ?>
